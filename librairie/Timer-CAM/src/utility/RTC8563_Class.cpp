...
void RTC8563_Class::setTime(const rtc_time_t& time) {
    uint8_t buf[] = {byteToBcd2(time.seconds), byteToBcd2(time.minutes), byteToBcd2(time.hours)};
...
void RTC8563_Class::setDate(const rtc_date_t& date) {
    uint8_t w = date.weekDay;
    if (w > 6 && date.year >= 1900 && ((size_t)(date.month - 1)) < 12) {  /// weekDay auto adjust
    ...
    uint8_t buf[] = {byteToBcd2(date.date), w,
                          (uint8_t)(byteToBcd2(date.month) + (date.year < 2000 ? 0x80 : 0)),
...
int RTC8563_Class::setAlarmIRQ(int afterSeconds) {
    uint8_t reg_value = _i2c.readByte(BM8563_I2C_ADDR, 0x01) & ~0x0C;
